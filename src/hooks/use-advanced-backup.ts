...
export interface BackupComputed {
  backupSummary: ReturnType<typeof getBackupSummary> | null;
...
export function useAdvancedBackup(
...
): BackupState & BackupActions & BackupComputed {
    ...
    const interval = setInterval(
      () => {
        ...
        if (lastBackupMeta) {
          ...
          if (lastBackup) {
            analyzeBackupHealth(lastBackup, getCurrentData()).then((health) => {
  ...
  const saveCreatedBackup = useCallback(
    async (destination: "download" | "share" | "cloud") => {
  ...
  const confirmRestore = useCallback(async () => {
    ...
    try {
      ...
      const restoredData = resolveEnvelopeData(finalBackup.data);
      if (!restoredData) {
        throw new Error("Restored payload could not be decoded.");
      }
      const { meta: m } = await createAdvancedBackup(restoredData);
      setLastBackupMeta(m);
  ...
  const setRestoreStep = useCallback((step: 0 | 1 | 2) => {
    setState((prev) => ({ ...prev, restoreStep: step }));
  }, []);
  ...
  const cleanupOldBackups = useCallback(async (maxAgeHours: number = 720) => {
    try {
      ...
      toast.success(
        `Cleaned up ${result.deleted} old backups, freed ${formatBytes(result.spaceFreed)}`,
      );
    } catch (error) {
  ...
  const backupSummary = useMemo(() => {
    ...
    const data = resolveEnvelopeData(state.currentBackup.data);
    return data ? getBackupSummary(data) : null;
  }, [state.currentBackup]);
...
export function useCloudBackup(provider?: CloudProvider) {
  ...
  const handleSync = useCallback(
    async (direction?: "upload" | "download" | "both") => {
...
export function useBackupHealth() {
  const { backupHealth, checkBackupHealth, lastBackupMeta, backupStatus } = useAdvancedBackup();
...
export function useBackupStorage() {
  const { storageStats, backups, cleanupOldBackups, optimizeStorage, deleteBackup } =
    useAdvancedBackup();
  ...
  const isOverQuota = useMemo(() => {
    return (storageStats?.quota.percentageUsed ?? 0) > 80;
  }, [storageStats]);
