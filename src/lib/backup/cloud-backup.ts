...
export interface CloudProviderInterface {
   ...
   */
  list(options?: { folderId?: string; limit?: number }): Promise<{
    success: boolean;
    backups?: Array<{
      id: string;
      name: string;
...
export class GoogleDriveProvider implements CloudProviderInterface {
  ...
  async upload(
    ...
    const filename =
      options.filename || `SkillSync-Backup-${new Date().toISOString().slice(0, 10)}.json`;

    // Generate checksum
    const checksum = await generateChecksum(JSON.stringify(backup));
...
export class GitHubGistProvider implements CloudProviderInterface {
  ...
  async upload(
    ...
    if ((backup.meta.sizeBytes ?? JSON.stringify(backup).length) > 100 * 1024 * 1024) {
...
export class CustomProvider implements CloudProviderInterface {
...
export class CloudBackupManager {
  ...
  private saveConfig(): void {
    try {
      localStorage.setItem(`skillsync:cloud:config:${this.deviceId}`, JSON.stringify(this.config));
    } catch {
      // Storage unavailable
    }
  }
...
export class CloudSyncManager {
  ...
  async syncAllDevices(
    ...
    const result = await manager.syncAll(options);
    return {
      success: result.success,
...
export type { CloudProvider, CloudBackupConfig };
